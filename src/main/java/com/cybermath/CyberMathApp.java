...
public class CyberMathApp extends Application {
    ...
    private void aplicarEfectoTema(Parent root) {
        if (jugador != null) {
            ...
            switch (jugador.getTemaUI()) {
                case "AMBAR": colorAdjust.setHue(-0.6); break;
                case "AZUL":  colorAdjust.setHue(0.8);  break;
                default:      colorAdjust.setHue(0);    break;
            }
    ...
    private Label estilo(String t, int size, String color) {
        Label l = new Label(t);
        l.setStyle("-fx-text-fill:" + color + ";-fx-font-size:" + size + "px;-fx-font-family:'Consolas';-fx-font-weight:bold;");
        return l;
    }

    private String estadoJugador() {
        return "OP: " + jugador.getNombre() + "   HP: " + Math.max(0, jugador.getIntegridad()) + "%" +
                "   BTC: " + jugador.getCriptos() + "   NODOS: " + jugador.getNivelesSuperados() + "/50";
    }
    ...
    private void mostrarMenuPrincipal() {
        ...
        // CORRECCIÓN DE RASTERIZACIÓN: Sin DropShadow en el VBox, solo bordes CSS manuales.
        panel.setStyle("-fx-background-color:rgba(0,18,0,0.90);" +
                "-fx-border-color:" + C_VERDE + ";-fx-border-width:2px;-fx-border-radius:6;-fx-background-radius:6;");

        Label lblT = new Label("CYBERMATH");
        lblT.setStyle("-fx-font-size:52px;-fx-font-weight:900;-fx-font-family:'Consolas';-fx-text-fill:" + C_VERDE + ";");
        Glow g = new Glow(0.9);
        DropShadow gds = new DropShadow(22, Color.web(C_VERDE)); gds.setInput(g);
        lblT.setEffect(gds); // Solo el título tiene efecto, mantiene el texto inferior nítido

        Label lblS = new Label(":: PROTOCOL ZERO ::");
        lblS.setStyle("-fx-font-size:15px;-fx-text-fill:" + C_CYAN + ";-fx-font-family:'Consolas';");
        Label lblV = new Label("v2.0 TACTICAL  ·  UDES");
        lblV.setStyle("-fx-font-size:11px;-fx-text-fill:#5f8f5f;-fx-font-family:'Consolas';");
        ...
        lblSt.setStyle("-fx-text-fill:#44ff44;-fx-font-size:11px;-fx-font-family:'Consolas';-fx-font-weight:bold;");
    ...
    private void animarFondoMatrix(Canvas canvas) {
        ...
        animacionMatrix = new Timeline(new KeyFrame(Duration.millis(55), e -> {
            ...
            gc.setFill(Color.rgb(2, 10, 2, 0.14)); gc.fillRect(0, 0, w, h);
        ...
        animacionMatrix.setCycleCount(Timeline.INDEFINITE); animacionMatrix.play();
    }

    private void detenerAnimacionMatrix() { if (animacionMatrix != null) animacionMatrix.stop(); }

    // =========================================================================
    // CONFIGURACIÓN (RESTAURADA) Y CRÉDITOS
    // =========================================================================
    ...
    // =========================================================================
    // SELECTOR SLOTS Y TARJETAS ANIMADAS
    // =========================================================================
        ...
        for (int i = 1; i <= 3; i++) {
            ...
            tarjeta.setOnMouseClicked(e -> {
                ...
                if (guardado != null) {
                    ...
                    // LLAMAMOS A LA NUEVA INTERFAZ CREADA DESDE CERO
                    audio.playInfiltrado();
    ...
    // =========================================================================
    // CREACIÓN DE PERFIL (INTERFAZ NATIVA CYBERPUNK)
    // =========================================================================
    private void mostrarCreacionPerfil(int slot) {
        ...
        // Estilo manual limpio para no rasterizar fuentes
        panel.setStyle("-fx-background-color: rgba(0, 18, 0, 0.90);" +
        ...
        // Auto-enfocar el campo de texto para escribir inmediatamente
    ...
    private VBox tarjetaSlot(int slot, Usuario u) {
        ...
        if (vacio) {
            ...
            c.getChildren().addAll(lblSl, estilo(u.getNombre(), 16, "white"), pb,
                    ...
                    estilo("Eficiencia: " + eficiencia, 12, C_MAGENTA), estilo("BTC: " + u.getCriptos(), 12, C_AMARILLO)
            );
        }
        return c;
    }
    ...
    // =========================================================================
    // MAPA, BÓVEDA (LETRAS CLARAS) Y NÚCLEO
    // =========================================================================

    private boolean isDesbloqueado(int nivel) {
        if (nivel == 1) return true;
        if (nivel == 2 || nivel == 12 || nivel == 22 || nivel == 32 || nivel == 42) return jugador.isNivelCompletado(1);
        return jugador.isNivelCompletado(nivel - 1);
    }

    private void mostrarMapaArbol() {
        BorderPane layout = new BorderPane(); layout.setStyle("-fx-background-color:" + C_FONDO + ";");
        ...
        leyenda.setPadding(new Insets(8)); leyenda.setStyle("-fx-background-color:rgba(0,10,0,0.85);");
        for (Object[] rama : RAMAS) leyenda.getChildren().add(itemLeyenda((String)rama[3], (String)rama[2]));
        layout.setBottom(leyenda);

        Pane mapaPane = new Pane(); mapaPane.setStyle("-fx-background-color:" + C_FONDO + ";");
        ...
        scroll.setFitToWidth(true); scroll.setStyle("-fx-background:" + C_FONDO + ";-fx-background-color:" + C_FONDO + ";");
        layout.setCenter(scroll);

        aplicarEfectoTema(layout);
        escenaPrincipal.setRoot(layout); fadeIn(mapaPane, 400);
    ...
    private void mostrarBovedaDatos() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            // CORRECCIÓN BÓVEDA: Evitamos la clase .cyber-panel para no heredar DropShadows globales
    ...
    private void dibujarNodoHex(Pane pane, double cx, double cy, int nivel, double r, boolean desbloqueado, String colorStr, String etiqueta) {
        ...
        if (desbloqueado) {
            ...
            hex.setEffect(new DropShadow(hecho ? 20 : 10, color)); // Sombra SOLO en el polígono
        } else {
        ...
        if (hecho) {
            ...
            // CORRECCIÓN NÚCLEO: Se centra visualmente con TranslateY y se usa blanco nítido
    ...
    private HBox itemLeyenda(String color, String texto) {
        ...
        r.setFill(Color.web(color).deriveColor(0, 1, 0.2, 0.9)); r.setStroke(Color.web(color)); r.setStrokeWidth(1.5);
    ...
    private void mostrarTienda() {
        ...
        ProgressBar pbHP = new ProgressBar(hp / 100.0); pbHP.setPrefWidth(180); pbHP.setStyle("-fx-accent:" + (hp > 60 ? C_VERDE : hp > 30 ? C_AMARILLO : C_ROJO) + ";");
        ...
        for (int i = 0; i < tienda.getCatalogo().size(); i++) {
            ...
            VBox info = new VBox(4); info.getChildren().addAll(estilo(it.getNombre(), 14, "white"), estilo("Costo: " + it.getPrecio() + " BTC", 13, C_AMARILLO));
            ...
            bc.setOnAction(e -> {
                if (tienda.comprar(idx, jugador).contains("EXITOSA")) { audio.playSuccess(); GestorDB.guardarUsuario(jugador, slotActual); } else audio.playError();
                mostrarTienda();
            });
    ...
    // =========================================================================
    // BRIEFING Y JUEGO (QTE Y DUPLICATE CHILDREN CORREGIDO)
    // =========================================================================
    ...
    private void mostrarBriefingMision(int nivel) {
        ...
        TextArea txt = new TextArea(); txt.setWrapText(true); txt.setEditable(false);
        txt.getStyleClass().add("text-area"); txt.setStyle("-fx-font-size:15px;"); txt.setPrefHeight(250); txt.setMaxWidth(720);

        int pregs = logica.getPreguntasPorNivel(nivel);
        HBox infoDif = new HBox(28); infoDif.setAlignment(Pos.CENTER); infoDif.getStyleClass().add("cyber-panel"); infoDif.setMaxWidth(500);
        infoDif.getChildren().addAll(
                estilo("SECUENCIAS: " + pregs,  13, C_CYAN), estilo("TIEMPO: " + (TIEMPO_MAX + jugador.getBonusTiempo()) + "s", 13, C_CYAN),
                estilo("PENALIZACIÓN: -34% HP",  13, C_ROJO)
        );

        Button btnI = btnAncho("[ EJECUTAR HACKEO ]", 300); btnI.setStyle(btnI.getStyle() + "-fx-font-size:15px;-fx-padding:11 0;"); btnI.setVisible(false);
        btnI.setOnAction(e -> iniciarJuego(nivel));
        Button btnC = btn("[ CANCELAR ]"); btnC.setOnAction(e -> mostrarMapaArbol());
        HBox bots = new HBox(20, btnC, btnI); bots.setAlignment(Pos.CENTER);

        root.getChildren().addAll(lTit, lRama, txt, infoDif, bots);
        aplicarEfectoTema(root); escenaPrincipal.setRoot(root); fadeIn(root, 300);

        escribirTextoAnimado(txt, logica.getDescripcionMision(nivel) + "\n\nDIFICULTAD: " + config.getDificultad(), () -> btnI.setVisible(true));
    }

    private void iniciarJuego(int nivel) {
        nivelActualJugando   = nivel; totalPreguntasActual = logica.getPreguntasPorNivel(nivel);
        preguntasPendientes  = totalPreguntasActual; alarmaSonando = false;

        lblHeader = new Label(); lblTimer = new Label(); barraTiempo = new ProgressBar(1.0);
        lblEnergia = new Label("ENERGÍA:"); lblEnergia.setStyle("-fx-text-fill:" + C_CYAN + ";-fx-font-size:12px;");
        barraEnergia = new ProgressBar(jugador.getEnergia() / 100.0); barraEnergia.setPrefWidth(100); barraEnergia.setStyle("-fx-accent:" + C_CYAN + ";");

        txtLog = new TextArea(); txtRespuesta = new TextField(); btnHack = new Button("[ EJECUTAR CÓDIGO ]  ↵");
        panelReto = new VBox(8); panelReto.setAlignment(Pos.CENTER);
        rootJuego = new VBox(14); rootJuego.getStyleClass().add("root"); rootJuego.setAlignment(Pos.TOP_CENTER); rootJuego.setPadding(new Insets(16));

        HBox hdr = new HBox(14); hdr.getStyleClass().add("cyber-panel"); hdr.setAlignment(Pos.CENTER); hdr.setPadding(new Insets(10, 20, 10, 20));
        lblHeader.setStyle("-fx-font-size:13px;-fx-font-family:'Consolas';"); lblTimer.setStyle("-fx-text-fill:" + C_ROJO + ";-fx-font-size:18px;-fx-font-weight:bold;");
        barraTiempo.setPrefWidth(200); actualizarHeader();

        // CORRECCIÓN DUPLICATE CHILDREN: Instancias separadas
        ...
        btnOverclock = btn("[ OVERCLOCK ] (50 EN)"); btnBruteForce = btn("[ BRUTE FORCE ] (100 EN)");
        ...
        btnOverclock.setOnAction(e -> activarOverclock()); btnBruteForce.setOnAction(e -> activarFuerzaBruta());
        ...
        txtLog.setEditable(false); txtLog.setPrefHeight(120); txtLog.setText("[SYSTEM] CONEXIÓN ESTABLECIDA — NODO " + nivel + "\n");
        ...
        txtRespuesta.setPromptText("_ INGRESE VALOR NUMÉRICO"); txtRespuesta.setMaxWidth(400); txtRespuesta.setAlignment(Pos.CENTER);
        btnHack.getStyleClass().add("button-hack"); btnHack.setPrefWidth(420); btnHack.setStyle(btnHack.getStyle() + "-fx-font-size:15px;-fx-padding:11 0;");
        btnHack.setDefaultButton(true); btnHack.setOnAction(e -> verificarRespuesta());
        Label lInp = new Label("INGRESE RESULTADO:"); lInp.setStyle("-fx-text-fill:" + C_CYAN + ";-fx-font-size:12px;");
        inp.getChildren().addAll(lInp, txtRespuesta, btnHack);

        rootJuego.getChildren().addAll(hdr, term, inp);
        aplicarEfectoTema(rootJuego); escenaPrincipal.setRoot(rootJuego);
        iniciarMineriaPasiva(); nuevoReto();
    }
    ...
    private void nuevoReto() {
        ...
        panelReto.getChildren().clear(); VBox vista = construirVistaReto(retoTexto); panelReto.getChildren().add(vista); fadeIn(vista, 220);
        txtRespuesta.clear(); txtRespuesta.setDisable(false); txtRespuesta.requestFocus(); iniciarTemporizador();
    }

    private void iniciarTemporizador() {
        if (timerAnimacion != null) timerAnimacion.stop();
        tiempoRestante = TIEMPO_MAX + jugador.getBonusTiempo();
        ...
        timerAnimacion = new Timeline(new KeyFrame(Duration.seconds(1), e -> {
            tiempoRestante--;
            double prog = (double) tiempoRestante / (TIEMPO_MAX + jugador.getBonusTiempo());
            lblTimer.setText(String.format("%02ds", tiempoRestante)); barraTiempo.setProgress(prog);
    ...
    private void verificarRespuesta() {
        ...
        try {
            ...
            if (logica.verificar(res)) {
                ...
                intentarDropLore();
                if (preguntasPendientes == 0) completarNodo(); else { txtLog.appendText("[SYSTEM] " + preguntasPendientes + " secuencias restantes.\n"); nuevoReto(); }
    ...
    private void completarNodo() {
        if (timerAnimacion != null) timerAnimacion.stop(); if (mineriaTimeline != null) mineriaTimeline.stop();
        jugador.sumarCriptos(100); jugador.completarNivel(nivelActualJugando); GestorDB.guardarUsuario(jugador, slotActual);
    ...
    private void procesarFallo(String motivo) {
        if (timerAnimacion != null) timerAnimacion.stop();
        audio.playError(); jugador.recibirDaño(); GestorDB.registrarHistorial(slotActual, nivelActualJugando, "FALLO", motivo);
        txtLog.appendText("[FAIL] ✗ " + motivo + " — -34% HP.\n"); GestorDB.guardarUsuario(jugador, slotActual); actualizarHeader();

        if (jugador.getIntegridad() <= 0) {
            if (mineriaTimeline != null) mineriaTimeline.stop();
            panelReto.getChildren().clear(); Label lf = new Label("⚠  SYSTEM FAILURE  ⚠");
            lf.setStyle("-fx-text-fill:" + C_ROJO + ";-fx-font-size:30px;-fx-font-weight:bold;"); lf.setEffect(new DropShadow(20, Color.web(C_ROJO)));
            panelReto.getChildren().add(lf); txtRespuesta.setDisable(true); btnHack.setDisable(true);
        ...
        } else { txtLog.appendText("[SYSTEM] HP: " + Math.max(0, jugador.getIntegridad()) + "%. Reintentando...\n"); iniciarTemporizador(); }
    }

    private void actualizarHeader() {
        if (lblHeader == null) return;
        double hp = jugador.getIntegridad(); String c = hp > 60 ? C_VERDE : hp > 30 ? C_AMARILLO : C_ROJO;
        lblHeader.setText(estadoJugador() + "   NODO: " + nivelActualJugando); lblHeader.setStyle("-fx-font-size:13px;-fx-font-family:'Consolas';-fx-text-fill:" + c + ";");
    }

    private void actualizarBotonesHabilidad() {
        barraEnergia.setProgress(jugador.getEnergia() / 100.0);
        btnOverclock.setDisable(jugador.getEnergia() < 50); btnBruteForce.setDisable(jugador.getEnergia() < 100);
    }
    ...
    private void activarOverclock() {
        ...
        if (timerAnimacion != null) {
            ...
            timerAnimacion = new Timeline(new KeyFrame(Duration.seconds(2), e -> {
                tiempoRestante--; double prog = (double) tiempoRestante / (TIEMPO_MAX + jugador.getBonusTiempo());
                lblTimer.setText(String.format("%02ds", tiempoRestante)); barraTiempo.setProgress(prog);
                if (tiempoRestante <= 0) procesarFallo("TIMEOUT");
            }));
            timerAnimacion.setCycleCount(Timeline.INDEFINITE); timerAnimacion.play();
    ...
    private void generarEventoRed() {
        ...
        if (tipo == 0) {
            Platform.runLater(() -> {
                ...
                txtRespuesta.setText(txtRespuesta.getText() + "0xERR"); txtRespuesta.positionCaret(txtRespuesta.getText().length()); audio.playAmenaza();
            });
        } else {
            Platform.runLater(() -> {
                txtLog.appendText("[WARN] ⚠ RASTREO CORPORATIVO: -3 Segundos.\n");
                tiempoRestante = Math.max(1, tiempoRestante - 3); audio.playAmenaza();
            });
        }
    }
    ...
    private void iniciarMineriaPasiva() {
        if (jugador != null && jugador.getMineros() > 0) {
            ...
            mineriaTimeline = new Timeline(new KeyFrame(Duration.seconds(10), e -> {
                int ganancia = jugador.getMineros() * 2; jugador.sumarCriptos(ganancia);
    ...
    private void iniciarBypassManual() {
        ...
        Label lblAlerta = new Label(">>> FIREWALL FÍSICO DETECTADO <<<"); lblAlerta.setStyle("-fx-text-fill:" + C_AMARILLO + ";-fx-font-size:18px;-fx-font-weight:bold;");
        Label lblInstruccion = new Label("SINCRONICE EL PULSO EN LA ZONA VERDE PARA HACER BYPASS"); lblInstruccion.setStyle("-fx-text-fill:white;-fx-font-size:13px;");
        ProgressBar escanner = new ProgressBar(0.0); escanner.setPrefWidth(400); escanner.setPrefHeight(25); escanner.setStyle("-fx-accent:" + C_CYAN + "; -fx-control-inner-background: #001100;");
        ...
        animacionBypass = new Timeline(new KeyFrame(Duration.millis(20), e -> {
            if (moviendoDerecha) { posicionBypass += 0.035; if (posicionBypass >= 1.0) moviendoDerecha = false; } else { posicionBypass -= 0.035; if (posicionBypass <= 0.0) moviendoDerecha = true; }
            escanner.setProgress(posicionBypass);
            escanner.setStyle("-fx-accent:" + ((posicionBypass >= 0.42 && posicionBypass <= 0.58) ? C_VERDE : C_CYAN) + "; -fx-control-inner-background: #001100;");
        }));
        ...
        btnInterceptar.setOnAction(e -> {
            ...
            if (posicionBypass >= 0.42 && posicionBypass <= 0.58) {
                audio.playSuccess(); txtLog.appendText("[OK] ✓ BYPASS FÍSICO EXITOSO. SECUENCIA SALTADA.\n");
                ...
                audio.playError(); txtLog.appendText("[FAIL] ✗ DESINCRONIZACIÓN. EL FIREWALL HA GOLPEADO.\n"); procesarFallo("FALLO DE REFLEJOS");
    ...
    private VBox construirVistaReto(String reto) {
        VBox cont = new VBox(8); cont.setAlignment(Pos.CENTER); String[] partes = reto.split("\n>>> ", 2);
        Label lPre = new Label(partes[0]); lPre.setStyle("-fx-text-fill:" + C_CYAN + ";-fx-font-size:13px;-fx-opacity:0.85;"); cont.getChildren().add(lPre);
        String cuerpo = partes.length > 1 ? partes[1] : "";
        if (cuerpo.contains("d/dx") || cuerpo.contains("∫")) {
            String[] lin = cuerpo.split("\n", 2); Label lEtq = new Label(lin[0]); lEtq.setStyle("-fx-text-fill:" + C_AMARILLO + ";-fx-font-size:15px;-fx-font-weight:bold;");
            cont.getChildren().addAll(lEtq, renderizarNotacion(lin.length > 1 ? lin[1].trim() : cuerpo));
        } else { Label lExp = new Label(cuerpo); lExp.setId("lblReto"); lExp.setWrapText(true); lExp.setTextAlignment(TextAlignment.CENTER); cont.getChildren().add(lExp); }
        return cont;
    }
    ...
    private VBox renderizarNotacion(String expr) {
        ...
        box.setStyle("-fx-background-color:rgba(0,25,0,0.6);-fx-border-color:" + C_CYAN + ";-fx-border-width:1.5px;-fx-border-radius:5;-fx-background-radius:5;");
        ...
        if (expr.contains("d/dx")) {
            ...
            String eval = expr.contains("para x=") ? "evaluado en  " + expr.substring(expr.indexOf("para x=")) : expr.contains("cuando x=") ? "evaluado en  " + expr.substring(expr.indexOf("cuando x=")) : "";
            HBox frac = new HBox(4); frac.setAlignment(Pos.CENTER); VBox df = new VBox(0); df.setAlignment(Pos.CENTER);
            df.getChildren().addAll(estilo("d", 22, "white"), new Label("──") {{ setStyle("-fx-text-fill:white;-fx-font-size:14px;"); }}, estilo("dx", 18, "white"));
            ...
            String limites = (expr.contains("[") && expr.contains("]")) ? expr.substring(expr.indexOf("[") + 1, expr.indexOf("]")).trim() : "";
            String[] lp = limites.split(" a "); String limInf = lp.length > 0 ? lp[0].trim() : "0", limSup = lp.length > 1 ? lp[1].trim() : "?", intg = "";
            ...
            HBox row = new HBox(4); row.setAlignment(Pos.CENTER); VBox simb = new VBox(0); simb.setAlignment(Pos.CENTER);
